...
//
//For fetching from Yahoo the data for charts
...
class ChartViewModel: ObservableObject {
    ...
    @Published var fetchPhase = Fetching<ChartDataV>.initial
    var chart: ChartDataV? { fetchPhase.value }
    ...
    @Published var selectedX: (any Plottable)?
        ...
        var foregroundMarkColor: Color {
            (selectedX != nil) ? .gray : (chart?.lineColor ?? .gray)
        }
        ...
        func transformChartViewData(_ data: ChartData) -> ChartDataV {
        ...
        func getLineColor(data: ChartData) -> Color {
            if let last = data.indicators.last?.close {
                if selectedRange == .oneDay, let prevClose = data.meta.previousClose {
                    return last >= prevClose ? .blue : .blue
                } else if let first = data.indicators.first?.close {
                    return last >= first ? .blue : .blue
                }
            }
            return .gray
        }
